...
Originally Created: August 26, 2018
...
def get_welcome_response(request, session):
    ...
    card_title = "Welcome to Math Geek."
    text = "Welcome to Math Geek, your personal math tutor.  " \
...
def play_game(intent, session):
    """
    Main function for Math Geek game
    """
...
def get_help(intent, session):
    ...
    card_title = "Math Geek Help"
    text = "Looking for help?  Math Geek is easy to use.  " \
...
def difficulty_menu(intent, session):
    ...
    '''
    card_title = "Math Geek Difficulty Settings"
    text = "Math Geek will get more challenging as you answer more questions correctly.  " \
...
def set_roundlength(intent, session):
    ...
    card_title = "Math Geek Round Length Setting"
    speech_output = "<speak>" \
                    + text + \
                    "<break time=\"500ms\" />" \
                    "Would you like to start a game?  " \
...
def setup_session_attributes():
    '''
    Sets up initial Math Geek Session Attributes if new session.
    '''
    session_attributes = {}
